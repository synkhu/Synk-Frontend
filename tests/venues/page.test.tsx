...
jest.mock('../../components/ProtectedRoute', () => ({
  __esModule: true,
  default: ({ children }: any) => <div data-testid="protected-content">{children}</div>,
}));
...
const { getVenues } = require('../../app/services/venue.service');

const mockVenues = [
  { id: 1, name: 'Test Venue', city: 'Test City', address: '123 Test St', country: 'Test Country', capacity: 1000, description: 'Test desc' },
];
...
describe('VenuesPage', () => {
  beforeEach(() => {
    ...
    authService.getSession.mockReturnValue({ user: null });
  });
  ...
  it('renders main content when authorized with empty venues', async () => {
    ...
    expect(within(wrapper).getByRole('heading', { name: /venues/i })).toBeInTheDocument();
    expect(within(wrapper).getByRole('button', { name: /new/i })).toBeInTheDocument();
  ...
  it('opens modal when New button clicked', async () => {
    ...
    const newButton = within(wrapper).getByRole('button', { name: /new/i });
    fireEvent.click(newButton);

    await waitFor(() => expect(screen.getByText('VenueForm')).toBeInTheDocument());
  });

  it('handles VenueForm success by updating venues', async () => {
    const updatedVenues = [
      ...mockVenues,
      { id: 2, name: 'New Venue', city: 'New City', address: 'New St', country: 'New Country', capacity: 2000, description: 'New' },
    ];

    authService.canAccessAdminPages.mockResolvedValueOnce(true);
    getVenues.mockResolvedValueOnce(mockVenues).mockResolvedValueOnce(updatedVenues);

    const MockVenueForm = require('../../app/venues/venueform').default;
    MockVenueForm.mockImplementation(({ onSuccess }: any) => {
