...
export default function AllEventsPage() {
  ...
  return (
    <div className="py-2 md:py-12 px-4 md:px-8 max-w-7xl mx-auto space-y-5 md:space-y-10">
      ...
      <div className="mb-4 md:mb-8 relative">
        {checkIsMobile(navigator.userAgent) ? (
          <section className="bg-white/5 border border-white/10 rounded-[2.5rem] p-2 shadow-xl backdrop-blur-md relative transition-all overflow-hidden">
            ...
            <div
              className={`overflow-hidden transition-[max-height,opacity] duration-500 ${showFilters ? "max-h-[2000px] opacity-100" : "max-h-0 opacity-0"
                }`}
            >
          ...
          <section className="bg-white/5 border border-white/10 rounded-[2.5rem] p-4 md:p-8 md:p-10 shadow-xl backdrop-blur-md">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">Price Range (HUF)</label>
                <div className="px-1 pt-2">
                  {(() => {
                    const minVal = parseInt(filters.minPrice || String(priceSliderMin));
                    const maxVal = parseInt(filters.maxPrice || String(priceSliderMax));
